...
void do_work(int t_int)
{
    ...
    LOG_INFO("Worker-%d thread [id: %d] start.", t_int, std::this_thread::get_id());
    ...
    while (!s_finished)
    {
        ...
        LOG_INFO("Worker-%d thread [id: %d] wake up after %fs ...",
                 t_int, std::this_thread::get_id(), duration.count());

        std::lock_guard lg{data.mtx};
        data.counter++;
        LOG_INFO("Worker-%d thread data.counter after increacement: %d", t_int, data.counter);
    }
    LOG_INFO("Worker-%d thread [id: %d] is finished.", t_int, std::this_thread::get_id());
}
