#include "editor/panels/toolbar.hpp"

#include "ui/widgets/layout/spacing.hpp"
// #include "editor/core/editor_actions.hpp"
...
Toolbar::Toolbar(const std::string& p_title, bool p_opened,
  ...
  // m_playButton	=
  // &CreateWidget<UI::Widgets::Button::ButtonImage>(EDITOR_CONTEXT(editorResources)->GetTexture("Button_Play")->id,
  // Maths::FVector2{ 20, 20 }); m_pauseButton	=
  // &CreateWidget<UI::Widgets::Button::ButtonImage>(EDITOR_CONTEXT(editorResources)->GetTexture("Button_Pause")->id,
  // Maths::FVector2{ 20, 20 }); m_stopButton	=
  // &CreateWidget<UI::Widgets::Button::ButtonImage>(EDITOR_CONTEXT(editorResources)->GetTexture("Button_Stop")->id,
  // Maths::FVector2{ 20, 20 }); m_nextButton	=
  // &CreateWidget<UI::Widgets::Button::ButtonImage>(EDITOR_CONTEXT(editorResources)->GetTexture("Button_Next")->id,
  // Maths::FVector2{ 20, 20 });

  // CreateWidget<UI::Widgets::Layout::Spacing>(0).lineBreak = false;
  // auto& refreshButton	=
  // CreateWidget<UI::Widgets::Buttons::ButtonImage>(EDITOR_CONTEXT(editorResources)->GetTexture("Button_Refresh")->id,
  // Maths::FVector2{ 20, 20 });
  ...
  // EDITOR_EVENT(EditorModeChangedEvent) +=
  // [this](OvEditor::Core::EditorActions::EEditorMode p_newMode)
  // {
  // 	auto enable = [](UI::Widgets::Buttons::ButtonImage* p_button, bool
  // p_enable)
  // 	{
  // 		p_button->disabled = !p_enable;
  // 		p_button->tint = p_enable ?
  // UI::Types::Color{ 1.0f, 1.0f, 1.0f, 1.0f} :
  // UI::Types::Color{1.0f, 1.0f, 1.0f, 0.15f};
  // 	};
