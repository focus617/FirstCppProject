...
bool TreeNode::is_opened() const { return m_opened; }
...
void TreeNode::_on_draw_impl() {
  ...
  if (ImGui::IsItemClicked() &&
    ...
    if (ImGui::IsMouseDoubleClicked(0)) {
      ...
      auto double_click_event = CreateRef<Events::DrawableDoubleClickedEvent>(
  ...
  if (opened) {
    ...
    execute_plugins();  // Manually execute plugins to make plugins considering
                        // the TreeNode and no childs
