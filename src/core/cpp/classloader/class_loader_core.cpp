...
namespace class_loader
{
    namespace impl
    {
        ...
        {
            LOG(INFO)
                << "class_loader.impl: Inserting MetaObject (class = "
                << meta_obj->className()
            ...
            const std::string &library_path, const ClassLoader *loader)
        {
            ...
            LOG(INFO)
                << "class_loader.impl: "
                << "Removing MetaObjects associated with library "
                << library_path
                << " and class loader "
                << reinterpret_cast<const void *>(loader)
                << " from global plugin-to-factorymap map.\n";

            // We have to walk through all FactoryMaps to be sure
            BaseToFactoryMapMap &factory_map_map =
        ...
        bool areThereAnyExistingMetaObjectsForLibrary(
            ...
            const std::string &library_path, ClassLoader *loader)
        {
            ...
            {
                LOG(INFO)
                    << "class_loader.impl: Tagging existing MetaObject "
            ...
            const std::string &library_path, ClassLoader *loader)
        {
            ...
            for (auto &obj : graveyard)
            {
                if (obj->getAssociatedLibraryPath() == library_path)
                {
                    LOG(INFO)
                        << "class_loader.impl: "
                        << "Resurrected factory metaobject from graveyard, class = " << obj->className()
                        << " , base_class = " << obj->baseClassName()
                        << " ptr = " << reinterpret_cast<void *>(obj)
                        << "..."
                        << "bound to ClassLoader "
...
#pragma GCC diagnostic pop
        ...
        void loadLibrary(const std::string &library_path, ClassLoader *loader)
        {
            ...
            LOG(INFO)
                << "class_loader.impl: Attempting to load library "
                << library_path
            ...
            if (isLibraryLoadedByAnybody(library_path))
            {
                ...
                LOG(INFO) << "class_loader.impl: Library already in memory, but binding existing MetaObjects to loader if necesesary.";
...

            {
                ...
                catch (const xuzy::LibraryLoadException &e)
                {
                    ...
                    throw xuzy::LibraryLoadException(
                        "Could not load library: " + std::string(e.message()));
                }
                catch (const xuzy::LibraryAlreadyLoadedException &e)
                {
                    setCurrentlyLoadingLibraryName("");
                    setCurrentlyActiveClassLoader(nullptr);
                    throw xuzy::LibraryAlreadyLoadedException(
                        "Library " + library_path + " already loaded (" + std::string(e.message()) + ")");
                }
                catch (const xuzy::NotFoundException &e)
                {
                    setCurrentlyLoadingLibraryName("");
                    setCurrentlyActiveClassLoader(nullptr);
                    throw xuzy::NotFoundException(
                        "Library not found (" + std::string(e.message()) + ")");
                }
            ...
            LOG(INFO)
                << "class_loader.impl: Successfully loaded library "
                << library_path
            ...
            {
                LOG(INFO)
                    << "class_loader.impl: Though the library "
                    << library_path
        ...
        void unloadLibrary(const std::string &library_path, ClassLoader *loader)
        {
            if (hasANonPurePluginLibraryBeenOpened())
            {
                LOG(INFO)
                    ...
                    << "As class_loader has no idea which libraries class factories were exported from, it can safely close any library without potentially unlinking symbols that are still actively being used. "
                    << "You must refactor your plugin libraries to be made exclusively of plugins in order for this error to stop happening.";
            }
            else
            {
                LOG(INFO)
                ...
                if (itr != open_libraries.end())
                {
                    ...
                    try
                    {
                        ...
                        if (!areThereAnyExistingMetaObjectsForLibrary(lib_path))
                        {
                            LOG(INFO)
                                << "class_loader.impl: "
                                << "There are no more MetaObjects left for "
                                << lib_path
                                << " so unloading library and removing from loaded library vector.\n";
                        ...
                        else
                        {
                            LOG(INFO)
                                << "class_loader.impl: "
                                << "MetaObjects still remain in memory meaning other ClassLoaders are still using library, keeping library " << lib_path << " open.";
                        }
                        return;
                    }
                    ...
                    {
                        delete (library);
                        throw xuzy::LibraryUnLoadException("Could not unload library (" + std::string(e.message()) + ")");
                    }
                }
                throw xuzy::LibraryUnLoadException("Attempt to unload library that class_loader is unaware of.");
            }
        }
