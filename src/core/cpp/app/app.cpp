...
App::App(const std::string& t_app_name, const std::string& t_version)
  ...
  m_specification_.m_app_name_ = t_app_name;
  m_specification_.m_version_ = t_version;

  LOG(INFO) << m_specification_.m_app_name_ << " version: " << m_specification_.m_version_ << std::endl;
}
...
void App::dumpError(std::string error) {
  LOG(ERROR) << m_specification_.m_app_name_ << " error: " << error << std::endl;
}
...
void App::load_conf(const std::string& filename) {
  ...
  LOG(INFO) << m_specification_.m_app_name_ << ": Load configuration from " << filename;
  App::get_impl()->load_conf_from_file(filename, m_conf_);
}

void App::setup() {
  // Setup configuration
  std::string config_filename = m_specification_.m_app_name_ + ".json";
