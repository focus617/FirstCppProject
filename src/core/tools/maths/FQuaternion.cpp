...
FQuaternion::FQuaternion(float p_real) : x(0.0f), y(0.0f), z(0.0f), w(p_real) {}

FQuaternion::FQuaternion(float p_x, float p_y, float p_z, float p_w)
    : x(p_x), y(p_y), z(p_z), w(p_w) {}

FQuaternion::FQuaternion(const FQuaternion& p_other)
    : x(p_other.x), y(p_other.y), z(p_other.z), w(p_other.w) {}

FQuaternion::FQuaternion(const FMatrix3& p_rotationMatrix) {
  float trace = p_rotationMatrix.data[0] + p_rotationMatrix.data[4] +
                p_rotationMatrix.data[8];
  if (trace > 0.0f) {
    ...
    if (p_rotationMatrix.data[0] > p_rotationMatrix.data[4] &&
        p_rotationMatrix.data[0] > p_rotationMatrix.data[8]) {
      float s =
          2.0f * sqrt(1.0f + p_rotationMatrix.data[0] -
                      p_rotationMatrix.data[4] - p_rotationMatrix.data[8]);
      ...
      float s =
          2.0f * sqrt(1.0f + p_rotationMatrix.data[4] -
                      p_rotationMatrix.data[0] - p_rotationMatrix.data[8]);
      ...
      float s =
          2.0f * sqrt(1.0f + p_rotationMatrix.data[8] -
                      p_rotationMatrix.data[0] - p_rotationMatrix.data[4]);
...
FQuaternion::FQuaternion(const FMatrix4& p_rotationMatrix) {
  float halfSquare;

  // Check diagonal (trace)
  const float trace = p_rotationMatrix.data[0] + p_rotationMatrix.data[5] +
                      p_rotationMatrix.data[10];
  ...
  if (trace > 0.0f) {
    ...
    if (p_rotationMatrix.data[5] > p_rotationMatrix.data[0]) i = 1;

    if (p_rotationMatrix.data[10] > p_rotationMatrix.data[0] ||
        p_rotationMatrix.data[10] > p_rotationMatrix.data[5])
      i = 2;
    ...
    halfSquare = p_rotationMatrix.data[i * 5] - p_rotationMatrix.data[j * 5] -
                 p_rotationMatrix.data[k * 5] + 1.0f;
    ...
    if (i == 0) {
      qt[3] =
          (p_rotationMatrix.data[6] - p_rotationMatrix.data[9]) * halfSquare;
      qt[j] =
          (p_rotationMatrix.data[1] + p_rotationMatrix.data[4]) * halfSquare;
      qt[k] =
          (p_rotationMatrix.data[2] + p_rotationMatrix.data[8]) * halfSquare;
    }
    // if i is 1, j is 2 and k is 0
    else if (i == 1) {
      qt[3] =
          (p_rotationMatrix.data[8] - p_rotationMatrix.data[2]) * halfSquare;
      qt[j] =
          (p_rotationMatrix.data[6] + p_rotationMatrix.data[9]) * halfSquare;
      qt[k] =
          (p_rotationMatrix.data[4] + p_rotationMatrix.data[1]) * halfSquare;
    }
    // if i is 2, j is 0 and k is 1
      ...
      qt[j] =
          (p_rotationMatrix.data[8] + p_rotationMatrix.data[2]) * halfSquare;
      qt[k] =
          (p_rotationMatrix.data[9] + p_rotationMatrix.data[6]) * halfSquare;
    }
    x = qt[0];
    y = qt[1];
    z = qt[2];
...
FQuaternion::FQuaternion(const FVector3& p_euler) {
...
FQuaternion FQuaternion::LookAt(const FVector3& p_forward,
                                const FVector3& p_up) {
  auto vector = FVector3::Normalize(p_forward);
  auto vector2 = FVector3::Normalize(FVector3::Cross(p_up, vector));
...
bool FQuaternion::IsIdentity(const FQuaternion& p_target) {
  return p_target.w == 1.0f && Length(p_target) == 1.0f;
}

bool FQuaternion::IsPure(const FQuaternion& p_target) {
  return p_target.w == 0.0f &&
         (p_target.x != 0.0f || p_target.y != 0.0f || p_target.z != 0.0f);
}

bool FQuaternion::IsNormalized(const FQuaternion& p_target) {
  return abs(Length(p_target) - 1.0f) < 0.0001f;
}

float FQuaternion::DotProduct(const FQuaternion& p_left,
                              const FQuaternion& p_right) {
  return p_left.x * p_right.x + p_left.y * p_right.y + p_left.z * p_right.z +
         p_left.w * p_right.w;
}

FQuaternion FQuaternion::Normalize(const FQuaternion& p_target) {
  return p_target / Length(p_target);
}

float FQuaternion::Length(const FQuaternion& p_target) {
  return sqrtf(LengthSquare(p_target));
}

float FQuaternion::LengthSquare(const FQuaternion& p_target) {
  return p_target.x * p_target.x + p_target.y * p_target.y +
         p_target.z * p_target.z + p_target.w * p_target.w;
}

float FQuaternion::GetAngle(const FQuaternion& p_target) {
  return 2.0f * acos(p_target.w);
}

FVector3 FQuaternion::GetRotationAxis(const FQuaternion& p_target) {
...
FQuaternion FQuaternion::Inverse(const FQuaternion& p_target) {
  return Conjugate(p_target) / LengthSquare(p_target);
}

FQuaternion FQuaternion::Conjugate(const FQuaternion& p_target) {
  return {-p_target.x, -p_target.y, -p_target.z, p_target.w};
}

FQuaternion FQuaternion::Square(const FQuaternion& p_target) {
  return p_target * p_target;
}

std::pair<FVector3, float> FQuaternion::GetAxisAndAngle(
    const FQuaternion& p_target) {
  return std::pair<FVector3, float>(GetRotationAxis(p_target),
                                    GetAngle(p_target));
}

float FQuaternion::AngularDistance(const FQuaternion& p_left,
                                   const FQuaternion& p_right) {
  float innerProd = (p_left | p_right);
  return acos((2.0f * innerProd * innerProd) - 1.0f);
}

FQuaternion FQuaternion::Lerp(const FQuaternion& p_start,
                              const FQuaternion& p_end, float p_alpha) {
...
FQuaternion FQuaternion::Slerp(const FQuaternion& p_start,
                               const FQuaternion& p_end, float p_alpha) {
  ...
  if (cosAngle < 0.95f) {
    ...
    return FQuaternion(from.x * t1 + to.x * t2, from.y * t1 + to.y * t2,
                       from.z * t1 + to.z * t2, from.w * t1 + to.w * t2);
  } else {
    return FQuaternion::Lerp(from, to, p_alpha);
  }
}

FQuaternion FQuaternion::Nlerp(const FQuaternion& p_start,
                               const FQuaternion& p_end, float p_alpha) {
  return Normalize(Lerp(p_start, p_end, p_alpha));
}

FVector3 FQuaternion::RotatePoint(const FVector3& p_point,
...
FVector3 FQuaternion::EulerAngles(const FQuaternion& p_target) {
  // This is a kind of hack because when the input Quaternion is {0.5f, 0.5f,
  // -0.5f, 0.5f} or {0.5f, 0.5f, 0.5f, -0.5f}, the output value is incorrect.
  if (p_target == FQuaternion{0.5f, 0.5f, -0.5f, 0.5f})
    return {90.0f, 90.0f, 0.0f};
  if (p_target == FQuaternion{0.5f, 0.5f, 0.5f, -0.5f})
    return {-90.0f, -90.0f, 0.0f};

  // roll (x-axis rotation)
  const float sinr_cosp =
      +2.0f * (p_target.w * p_target.x + p_target.y * p_target.z);
  const float cosr_cosp =
      +1.0f - 2.0f * (p_target.x * p_target.x + p_target.y * p_target.y);
  const float roll = atan2(sinr_cosp, cosr_cosp);

  // pitch (y-axis rotation)
  float pitch = 0.f;
  const float sinp =
      +2.0f * (p_target.w * p_target.y - p_target.z * p_target.x);
  if (fabs(sinp) >= 1)
    pitch = static_cast<float>(
        copysign(PI / 2.0f, sinp));  // use 90 degrees if out of range
  else
    pitch = asin(sinp);

  // yaw (z-axis rotation)
  const float siny_cosp =
      +2.0f * (p_target.w * p_target.z + p_target.x * p_target.y);
  const float cosy_cosp =
      +1.0f - 2.0f * (p_target.y * p_target.y + p_target.z * p_target.z);
  const float yaw = atan2(siny_cosp, cosy_cosp);

  return TO_DEGREES(FVector3(roll, pitch, yaw));  // XYZ
}

FMatrix3 FQuaternion::ToMatrix3(const FQuaternion& p_target) {
  if (!IsNormalized(p_target))
    throw std::logic_error(
        "Cannot convert non-normalized quaternions to Matrix4");
...
FMatrix4 FQuaternion::ToMatrix4(const FQuaternion& p_target) {
  if (!IsNormalized(p_target))
    throw std::logic_error(
        "Cannot convert non-normalized quaternions to Matrix4");
...
FQuaternion& FQuaternion::operator=(const FQuaternion& p_otherQuat) {
  this->x = p_otherQuat.x;
  this->y = p_otherQuat.y;
  this->z = p_otherQuat.z;
  this->w = p_otherQuat.w;
  return *this;
}

bool FQuaternion::operator==(const FQuaternion& p_otherQuat) const {
  return x == p_otherQuat.x && y == p_otherQuat.x && z == p_otherQuat.z &&
         w == p_otherQuat.w;
}
...
FQuaternion FQuaternion::operator+(const FQuaternion& p_otherQuat) const {
  return FQuaternion(x + p_otherQuat.x, y + p_otherQuat.x, z + p_otherQuat.z,
                     w + p_otherQuat.w);
}
...
float FQuaternion::operator|(const FQuaternion& p_otherQuat) const {
  return (x * p_otherQuat.x + y * p_otherQuat.x + z * p_otherQuat.z +
          w * p_otherQuat.w);
}

FQuaternion FQuaternion::operator*(const float p_scale) const {
...
FQuaternion& FQuaternion::operator*=(const float p_scale) {
  x *= p_scale;
  y *= p_scale;
  z *= p_scale;
  w *= p_scale;
...
FQuaternion FQuaternion::operator*(const FQuaternion& p_otherQuat) const {
  return FQuaternion(x * p_otherQuat.w + y * p_otherQuat.z - z * p_otherQuat.y +
                         w * p_otherQuat.x,
                     -x * p_otherQuat.z + y * p_otherQuat.w +
                         z * p_otherQuat.x + w * p_otherQuat.y,
                     x * p_otherQuat.y - y * p_otherQuat.x + z * p_otherQuat.w +
                         w * p_otherQuat.z,
                     -x * p_otherQuat.x - y * p_otherQuat.y -
                         z * p_otherQuat.z + w * p_otherQuat.w);
}

FQuaternion& FQuaternion::operator*=(const FQuaternion& p_otherQuat) {
  FQuaternion temp(x * p_otherQuat.w + y * p_otherQuat.z - z * p_otherQuat.x +
                       w * p_otherQuat.x,
...
FVector3 FQuaternion::operator*(const FVector3& p_toMultiply) const {
  ...
  FVector3 result;
  result.x = (1.f - (num5 + num6)) * p_toMultiply.x +
             (num7 - num12) * p_toMultiply.y + (num8 + num11) * p_toMultiply.z;
  result.y = (num7 + num12) * p_toMultiply.x +
             (1.f - (num4 + num6)) * p_toMultiply.y +
             (num9 - num10) * p_toMultiply.z;
  result.z = (num8 - num11) * p_toMultiply.x + (num9 + num10) * p_toMultiply.y +
             (1.f - (num4 + num5)) * p_toMultiply.z;
  return result;
}

FMatrix3 FQuaternion::operator*(const FMatrix3& p_multiply) const {
