#include "tools/maths/FTransform.hpp"

namespace xuzy::Maths {

FTransform::FTransform(FVector3 p_localPosition, FQuaternion p_localRotation,
                       FVector3 p_localScale)
    : m_notificationHandlerID(-1), m_parent(nullptr) {
  GenerateMatricesLocal(p_localPosition, p_localRotation, p_localScale);
}

FTransform::~FTransform() {
  Notifier.NotifyChildren(
      Internal::TransformNotifier::ENotification::TRANSFORM_DESTROYED);
}

void FTransform::NotificationHandler(
    Internal::TransformNotifier::ENotification p_notification) {
  switch (p_notification) {
    ...
    case Internal::TransformNotifier::ENotification::TRANSFORM_DESTROYED:
      /*
       * RemoveParent() is not called here because it is unsafe to remove a
       * notification handler while the parent is iterating on his notification
       * handlers (Segfault otherwise)
       */
...
bool FTransform::HasParent() const { return m_parent != nullptr; }

void FTransform::GenerateMatricesLocal(FVector3 p_position,
                                       FQuaternion p_rotation,
                                       FVector3 p_scale) {
  m_localMatrix = FMatrix4::Translation(p_position) *
                  FQuaternion::ToMatrix4(FQuaternion::Normalize(p_rotation)) *
                  FMatrix4::Scaling(p_scale);
...
void FTransform::GenerateMatricesWorld(FVector3 p_position,
                                       FQuaternion p_rotation,
                                       FVector3 p_scale) {
  m_worldMatrix = FMatrix4::Translation(p_position) *
                  FQuaternion::ToMatrix4(FQuaternion::Normalize(p_rotation)) *
                  FMatrix4::Scaling(p_scale);
...
void FTransform::UpdateWorldMatrix() {
  m_worldMatrix =
      HasParent() ? m_parent->m_worldMatrix * m_localMatrix : m_localMatrix;
  PreDecomposeWorldMatrix();

  Notifier.NotifyChildren(
      Internal::TransformNotifier::ENotification::TRANSFORM_CHANGED);
}

void FTransform::UpdateLocalMatrix() {
  m_localMatrix =
      HasParent() ? FMatrix4::Inverse(m_parent->m_worldMatrix) * m_worldMatrix
                  : m_worldMatrix;
...
void FTransform::ScaleLocal(const FVector3& p_scale) {
  SetLocalScale(FVector3(m_localScale.x * p_scale.x, m_localScale.y * p_scale.y,
                         m_localScale.z * p_scale.z));
}

const FVector3& FTransform::GetLocalPosition() const { return m_localPosition; }

const FQuaternion& FTransform::GetLocalRotation() const {
  return m_localRotation;
}

const FVector3& FTransform::GetLocalScale() const { return m_localScale; }

const FVector3& FTransform::GetWorldPosition() const { return m_worldPosition; }

const FQuaternion& FTransform::GetWorldRotation() const {
  return m_worldRotation;
}

const FVector3& FTransform::GetWorldScale() const { return m_worldScale; }

const FMatrix4& FTransform::GetLocalMatrix() const { return m_localMatrix; }

const FMatrix4& FTransform::GetWorldMatrix() const { return m_worldMatrix; }
...
void FTransform::PreDecomposeWorldMatrix() {
  ...
  FMatrix3 rotationMatrix(columns[0].x, columns[1].x, columns[2].x,
