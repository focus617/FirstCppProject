...
namespace class_loader
{
    ...
    namespace impl
    {
        ...
        /**
         * @brief Gets a handle to a global data structure that holds a map of base class names (Base class describes plugin interface) to a FactoryMap which holds the factories for the various different concrete classes that can be instantiated.
         * Note that the Base class is NOT THE LITERAL CLASSNAME, but rather the result of typeid(Base).name() which sometimes is the literal class name (as on Windows) but is often in mangled form (as on Linux).
         ...
         * Classes that use that macro will cause this function to be invoked when the library is loaded. The function will create a MetaObject (i.e. factory) for the corresponding Derived class and insert it into the appropriate FactoryMap in the global Base-to-FactoryMap map.
        ...
        void registerPlugin(const std::string &class_name, const std::string &base_class_name)
        {
            ...
            LOG(INFO)
                << "class_loader.impl: Registering plugin factory for class = "
                << class_name.c_str()
                << ", ClassLoader* = " << getCurrentlyActiveClassLoader()
            ...
            if (nullptr == getCurrentlyActiveClassLoader())
            {
                LOG(WARNING)
                    ...
                    << "This can happen if you build plugin libraries that contain more than just plugins (i.e. normal code your app links against). "
                    << "This inherently will trigger a dlopen() prior to main() and cause problems as class_loader is not aware of plugin factories that autoregister under the hood. "
                    << "The class_loader package can compensate, but you may run into namespace collision problems (e.g. if you have the same plugin class in two different libraries and you load them both at the same time). "
                    << "The biggest problem is that library can now no longer be safely unloaded as the ClassLoader does not know when non-plugin code is still in use. "
                    << "In fact, no ClassLoader instance in your application will be unable to unload any library once a non-pure one has been opened. "
            ...
            if (factoryMap.find(class_name) != factoryMap.end())
            {
                LOG(WARNING)
                    ...
                    << "A namespace collision has occurred with plugin factory for class " << class_name << ". "
            ...
            LOG(INFO)
                << "class_loader.impl: Registering of " << class_name
        ...
        Base *createInstance(const std::string &derived_class_name, ClassLoader *loader)
        {
            ...
            {
                LOG(ERROR)
                    << "class_loader.impl: No metaobject exists for class type "
                    << derived_class_name;
            }
            getPluginBaseToFactoryMapMapMutex().unlock();
            ...
            { // Was never created
                if (factory && factory->isOwnedBy(nullptr))
                {
                    LOG(INFO)
                        ...
                        << "This implies that the library containing the class was dlopen()ed by means other than through the class_loader interface. "
                        << "This can happen if you build plugin libraries that contain more than just plugins (i.e. normal code your app links against) -- that intrinsically will trigger a dlopen() prior to main(). "
                        << "You should isolate your plugins into their own library, otherwise it will not be possible to shutdown the library!";

                    obj = factory->create();
                }
                else
                {
                    throw xuzy::CreateClassException("Could not create instance of type " + derived_class_name);
                }
            }

            LOG(INFO)
                << "class_loader.impl: Created instance of type "
                << typeid(obj).name()
                << " and object pointer = "
        ...
        /**
         * @brief This function returns all the available classes in the plugin system that are derived from Base and within scope of the passed ClassLoader.
