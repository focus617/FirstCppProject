...
#  include "shared_library_win.hpp"
#endif

namespace class_loader {
/**
 * @brief The SharedLibrary class dynamically loads shared libraries at
 * run-time.
 */
class XUZY_API SharedLibrary : private SharedLibraryImpl {
 public:
  ...
  /**
   * @brief Creates a SharedLibrary object and loads a library from the given
   * path.
   */
  XUZY_API
  ...
  /**
   * @brief Creates a SharedLibrary object and loads a library from the given
   * path, using the given flags. See the Flags enumeration for valid values.
   */
  XUZY_API
   ...
   * Throws a LibraryAlreadyLoadedException if a library has already been
   * loaded. Throws a LibraryLoadException if the library cannot be loaded.
   */
  XUZY_API
  void load(const std::string& path);

  /**
   * @brief Loads a shared library from the given path, using the given flags.
  ...
  /**
   * @brief
   * @return the platform-specific filename prefix for shared libraries
  ...
  /**
   * @brief
   * @return the platform-specific filename suffix for shared libraries
   * (including the period). In debug mode, the suffix also includes a "d" to
   * specify the debug version of a library (e.g., "d.so", "d.dll") unless the
   * library has been compiled with -DPOCO_NO_SHARED_LIBRARY_DEBUG_SUFFIX.
   */
   ...
   * On all platforms the output of class_loader::systemLibrarySuffix() is
   * appended.
   */
  static std::string systemLibraryFormat(const std::string& library_name);

  /**
   * @brief Adds the given path to the list of paths shared libraries are
   * searched in.
   * @return true if the path was set, otherwise false.
   *
   * Currently only supported on Windows, where it calls SetDllDirectory(). On
   * all other platforms, does not do anything and returns false.
   */
   ...
   * @param args Arguments to pass to the actual class construction (arbitrary
   * length)
