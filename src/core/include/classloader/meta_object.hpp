 ...
 * @brief A base class for MetaObjects that excludes a polymorphic type
 * parameter. Subclasses are class templates though.
 */
class XUZY_API AbstractMetaObjectBase {
 public:
  ...
  /**
   * @brief Indicates if the factory is within the usable scope of any
   * ClassLoader
   */
...
class MetaObject : public AbstractMetaObject<B> {
 public:
  ...
  /**
   * @brief The factory interface to generate an object. The object has type C
   * in reality, though a pointer of the base class type is returned.
   * @return A pointer to a newly created plugin with the base class type (type
   * parameter B)
   */
