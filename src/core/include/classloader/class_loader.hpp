...
namespace class_loader
{
    ...
    private:
        ...
        {
            LOG(INFO) << "class_loader::ClassLoader: Calling onPluginDeletion() for obj ptr = " << reinterpret_cast<void *>(obj) << ".\n";
            ...
            if (0 == plugin_ref_count_ && isOnDemandLoadUnloadEnabled())
            {
                ...
                else
                {
                    LOG(WARNING)
                        ...
                        << " This is because createUnmanagedInstance was used within the scope of this process, perhaps by a different ClassLoader. Library will NOT be closed.";
                }
            }
        }
            ...
            const std::string &derived_class_name, bool managed)
        {
                ...
                isOnDemandLoadUnloadEnabled())
            {
                LOG(INFO)
                    << "class_loader::ClassLoader: "
                    << "An attempt is being made to create a managed plugin instance (i.e. boost::shared_ptr), however an unmanaged instance was created within this process address space. "
                    << "This means libraries for the managed instances will not be shutdown automatically on final plugin destruction if on demand (lazy) loading/unloading mode is used.";
            }
            if (!isLibraryLoaded())
            {
                loadLibrary();
            ...
            Base *obj = class_loader::impl::createInstance<Base>(
                derived_class_name, this);
